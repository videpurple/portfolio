...
| ![퍼널단계표시.png](./시각화자료/회원가입후이용절차.png) | 1. 회원 가입하기(create_user) <br> &nbsp;  - 회원 가입을 위한 가입 페이지 이동 <br> 2. 이메일 주소 확인(enter_email) <br> &nbsp;  - 이메일 주소를 이용하여 본인 확인 <br> 3. 회원 정보 입력(enter_info) <br> &nbsp;  - 이름 등 개인 정보 입력 <br> 4. 회원 가입 완료(complete_signup) <br> &nbsp;  - 회원 가입 완료 버튼을 누르면 가입 완료 페이지로 이동 |
|:---|:---|
